...
export default function EditDepartmentPage() {
  ...
  // Load department and warehouses on mount
  useEffect(() => {
    loadDepartment()
    loadWarehouses()
  }, [departmentId])
  ...
  const loadDepartment = async () => {
    try {
      ...
      setFormData({
        ...
        warehouses: linkedWarehouses.map((dw: DepartmentWarehouse) => ({
      ...
      setError("فشل تحميل بيانات القسم")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const addWarehouse = () => {
    ...
    const newWarehouse: WarehouseLink = {
      ...
      isPrimary: (formData.warehouses?.length || 0) === 0,
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await departmentService.updateDepartment(departmentId, formData)
      router.push("/departments/manage")
    } catch (err) {
      setError(err instanceof Error ? err.message : "حدث خطأ أثناء تحديث القسم")
    } finally {
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold">تعديل القسم</h1>
        <p className="text-muted-foreground">
          تعديل بيانات القسم والمستودعات المرتبطة
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              إدارة المستودعات التي يمكن للقسم الطلب منها
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Add Warehouse */}
